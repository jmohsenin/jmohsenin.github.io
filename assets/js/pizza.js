...
const updateIndicator = item => {
    ...
    // Get the current item by dividing the amount scrolled by the total width and multiplying it by the number of elements
        const activePhoto = Math.round(item.scrollLeft / item.scrollWidth * item.childElementCount)
...
*/
const openOverlay = photo => {
    const entry = photo.split('--')[0]
    ...
    const photoOffset = document.getElementById(photo).offsetLeft
